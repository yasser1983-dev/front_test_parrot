...
export const useOrderDispatch = (dishes: Dish[]) => {
    ...
    const dishTotalCost = (quantityValue: Number, itemIdOptional: Number = 0) => {
        ...
        if(itemId > 0 || itemIdOptional > 0){
            const selectedDish = dishes.find((dish: Dish) => dish.id === id);
            if (selectedDish) {
    ...
    const setDish = (itemId: Number) => {
        const selectedDish = dishes.find((dish: Dish) => dish.id === itemId);
